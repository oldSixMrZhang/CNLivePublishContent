//
//  CNLiveQQEmotionManager.m
//  testIM
//
//  Created by 张旭 on 2018/10/31.
 ...
 *  加载Emoji图片资源失败标识
 */
BOOL isLoadEmojiFailed;
BOOL isLoadingEmoji;//是否正在加载emoji
long CNLiveEmojiLoadFailCount = 0;
...
@protocol CNLiveQQEmotionInputViewProtocol <UITextInput>

@property(nonatomic, copy) NSString *text;
@property(nonatomic, assign, readonly) NSRange selectedRange;
@end

@implementation CNLiveQQEmotionManager

- (instancetype)init {
    self = [super init];
    if (self) {
        _emotionView = [[QMUIEmotionView alloc] init];
        self.emotionView.emotions = [CNLiveQQEmotionManager emotionsForQQ];
        __weak CNLiveQQEmotionManager *weakSelf = self;
...
- (UIView<CNLiveQQEmotionInputViewProtocol> *)boundInputView {
    if (self.boundTextField) {
        return (UIView<CNLiveQQEmotionInputViewProtocol> *)self.boundTextField;
    } else if (self.boundTextView) {
        return (UIView<CNLiveQQEmotionInputViewProtocol> *)self.boundTextView;
    }
    return nil;
}
...
+ (UIImage *)imageForQQEmotionWithIdentifier:(NSString *)identifier {
    return [QMUIHelper imageInBundle:[QMUIHelper resourcesBundleWithName:CNLiveResourcesQQEmotionBundleName] withName:identifier];
}

+ (NSArray<QMUIEmotion *> *)emotionsForQQ {
    if (QQEmotionArray) {
        if (!isLoadEmojiFailed)
        {//全部加载成功
            return QQEmotionArray;
        }else
        {//有图片加载失败
            NSMutableArray<QMUIEmotion *> *emotionsTemp = [NSMutableArray arrayWithArray:QQEmotionArray];
            if(!isLoadingEmoji)
            {
                [self asyncLoadImages:emotionsTemp];
            }
            return QQEmotionArray;
        }
    }
...
+ (void)asyncLoadImages:(NSArray<QMUIEmotion *> *)emotions{
    ...
    dispatch_async(dispatch_get_global_queue(0, 0), ^{
        for (QMUIEmotion *e in emotions) {
            if (e.image == nil) {
                UIImage *image = [QMUIEmotion cnliveImageWithEmotion:e];
                if (image == nil) {
                    NSLog(@"加载emoji失败id:%@",e.identifier);
                    isLoadEmojiFailed = YES;
                }else
                {
                    e.image = image;
                    isLoadEmojiFailed = NO;
                }
            }
        }
        isLoadingEmoji = NO;
