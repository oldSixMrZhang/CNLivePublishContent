 ...
 *  @warning 一个`CNLiveQQEmotionManager`无法同时绑定`boundTextField`和`boundTextView`，在两者都绑定的情况下，优先使用`boundTextField`。
 *  @warning 由于`CNLiveQQEmotionManager`里面多个地方会调用`boundTextView.text`，而`setText:`并不会触发`UITextViewDelegate`的`textViewDidChange:`或`UITextViewTextDidChangeNotification`，从而在刷新表情面板里的发送按钮的enabled状态时可能不及时，所以`CNLiveQQEmotionManager`要求绑定的`QMUITextView`必须打开`shouldResponseToProgrammaticallyTextChanges`属性
 */
@interface CNLiveQQEmotionManager : NSObject
...
/**
 *  显示QQ表情的表情面板，已被设置了默认的`didSelectEmotionBlock`和`didSelectDeleteButtonBlock`，在`CNLiveQQEmotionManager`初始化完后，即可将`emotionView`添加到界面上。
 */
